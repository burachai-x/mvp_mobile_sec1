...
> **แอปทำตามข้อนี้แล้ว** — ข้อความทั้งหมดอยู่ใน `apps/mobile/lib/l10n/app_th.arb`
> และ `app_en.arb` ครบทั้งสองภาษา ค่าเริ่มต้นเป็น `th` · ไฟล์ `app_localizations*.dart`
> ถูก generate แล้ว commit ไว้ เพราะ repo นี้ไม่มี CI ที่จะ generate ให้ตอน clone (ADR 0008)
> แก้ข้อความแล้วต้องรัน `flutter gen-l10n` ใหม่ทุกครั้ง
> เทสต์ยึดกับ key ผ่าน `AppLocalizations.delegate.load()` ไม่ยึดกับตัวข้อความ
> การแก้คำจึงไม่ทำให้เทสต์พังเหมือนการแก้พฤติกรรม
