...
### Security
- **`.jks` ย้ายไปอยู่บน "เครื่อง release เฉพาะ"** แทน CI secret ที่ไม่มีแล้ว (ADR 0008) —
  ดิสก์เข้ารหัส, ไฟล์ไม่ออกจากเครื่อง, password เก็บแยกใน password manager, ระบุผู้ถือเป็นชื่อคน
- **🔴 threat model S8 เพิ่มความเสี่ยงที่ยังเปิดอยู่ 2 ข้อ** — keystore **หาย** (เครื่องเดียวพัง = เซ็น APK ใหม่
  ไม่ได้อีกเลย ต้องติดตั้งใหม่ทั้งฐาน) จึงบังคับให้มี backup เข้ารหัส 2 ที่ที่ **ทดสอบกู้จริงก่อน release แรก**
